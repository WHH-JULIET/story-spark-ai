...
export default function AnalyticsDashboard() {
  ...
  // ✅ FIX: use the correct storage key — was localStorage.getItem("token")
  // but the app stores the JWT under AUTH_KEY = "accessToken"
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error(e);
        setError("Failed to load analytics. Please make sure you are logged in.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (error)
    return (
      <div className="min-h-screen bg-[#0d0d14] flex items-center justify-center">
        <div className="text-red-400 text-lg text-center px-6">{error}</div>
      </div>
    );

  // ✅ FIX: guard against empty hours array — hours[0] is undefined when array is []
  // which caused: TypeError: Cannot read properties of undefined (reading 'count')
  const maxHour =
    hours.length > 0
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            ...
            {maxHour && (
              <p className="text-xs text-white/40 mb-4">
                You write best at {HOUR_LABELS[maxHour.hour]}
              </p>
            )}
            {hours.length === 0 ? (
              <p className="text-white/30 text-center py-8">
            ...
            <div className="flex flex-wrap gap-2 justify-center py-4">
              {wordCloud.map((word, i) => {
                const size = Math.max(
                  ...
                  Math.min(
                    36,
                    12 + (word.value / wordCloud[0].value) * 24
                  )
                );
                return (
                  <span
                    ...
                    style={{
                      ...
                      opacity:
                        0.7 + (word.value / wordCloud[0].value) * 0.3,
                    }}
